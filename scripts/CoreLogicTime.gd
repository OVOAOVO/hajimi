...
@export var coin_count: int = 15            # 初始随机撒的金币数量
...
@export var _time_remaining: float = 0
...
func _ready() -> void:
	...
	var panel := $CanvasLayer/CountPanelTime/BoxContainer/TextureRect
	if panel:
		_current_value_label = panel.get_node_or_null("currentValue")

	# 查找倒计时 Label
	var panel2 := $CanvasLayer/CountPanelTime/BoxContainer2/TextureRect
	if panel2:
		_time_value_label = panel2.get_node_or_null("TimeValue")

	# 随机撒金币
	_spawn_coins()
	# 初始化 UI 为 0
	if _current_value_label:
		_current_value_label.text = "0"
...
## 金币无限生成：在倒计时期间每隔 coin_respawn_interval 秒生成一个金币
func _update_coin_spawn(delta: float) -> void:
	if not _timer_running:
		return
	_coin_spawn_timer += delta
	if _coin_spawn_timer >= coin_respawn_interval:
		_coin_spawn_timer = 0.0
		_spawn_single_coin()

## 倒计时更新：每秒减 1，到 0 时显示结算界面
func _update_timer(delta: float) -> void:
	if not _timer_running:
		return
	var prev := int(ceil(_time_remaining))
	_time_remaining -= delta
	var curr := int(ceil(_time_remaining))
	# 只在秒数变化时刷新 UI
	if curr != prev and _time_value_label:
